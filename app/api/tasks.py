...
def analyze_portfolio_task(db_task_id):
    try:
        ...
        for ticker in tickers_list:
            ...
            # 2. Download news articles
            ticker_obj = yf.Ticker(ticker)
            recent_news = ticker_obj.news[:3]
            for news_item in recent_news:
                # FIX HERE: extract the title from the 'content' sub-dictionary
                title = news_item.get('content', {}).get('title') or news_item.get('title', '')
                
                # Only add it if there is actually text
                if title:
        ...
        # 3. The hallucination-proof prompt (strict RAG)
        prompt = f"""
